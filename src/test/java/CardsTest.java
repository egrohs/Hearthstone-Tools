...
public class CardsTest {
	...
	public void testCardsTags() {
		...
		String[] cutsa = { "Corpsetaker", "Jan'alai, the Dragonhawk", "Zandalari Templar", "Shudderwock",
				"Tess Greymane", "Enhance-o Mechano", "Master Jouster", "Shaku, the Collector", "Gral, the Shark",
				"Shirvallah, the Tiger", "Curious Glimmerroot", "Lady in White", "Yogg-Saron, Hope's End",
				"Spirit of the Frog", "Cult Sorcerer", "Mecha'thun", "Doomcaller", "Spirit of the Dead",
				"Tol'vir Stoneshaper", "Bomb Squad", "Countess Ashmore", "The Darkness", "Jade Idol",
				"Leyline Manipulator", "Blade of C'Thun", "Genn Greymane", "Seaforium Bomber", "Da Undatakah",
				"Frost Lich Jaina", "Explosive Runes", "Spirit of the Tiger", "Myra Rotspring", "Lilian Voss",
				"Lord Godfrey", "Inkmaster Solia", "Tuskarr Jouster", "Stone Sentinel", "Marin the Fox",
				"Drakonid Operative", "Twin Emperor Vek'lor", "Omega Mind", "Spirit of the Shark", "Spiteful Summoner",
				"Iron Juggernaut", "Finja, the Flying Star", "Gilnean Royal Guard", "Ancient Shade",
				"Spirit of the Rhino", "Ice Block", "Zul'jin", "Steam Surger", "Geosculptor Yip", "Chillmaw",
				"Crazed Worshipper", "Glitter Moth", "Rattling Rascal", "Tinkmaster Overspark", "The Skeleton Knight",
				"The Lich King", "Twilight Acolyte", "Dragoncaller Alanna", "Cenarius", "Druid of the Saber",
				"Blood Imp", "Splintergraft", "Tomb Lurker", "Freezing Trap", "Raza the Chained", "Lone Champion",
				"Mind Control Tech", "Lesser Amethyst Spellstone", "Shellshifter", "Prince Liam", "Ivory Knight",
				"Servant of Yogg-Saron", "Mimiron's Head", "Druid of the Claw", "C'Thun", "Black Cat", "King Togwaggle",
				"Fireworks Tech", "High Priestess Jeklik", "The Voraxx", "Belligerent Gnome", "Book Wyrm",
				"Malfurion the Pestilent", "Misdirection", "Deathlord", "Ozruk", "Star Aligner", "Fal'dorei Strider",
				"Druid of the Swarm", "C'Thun's Chosen", "Servant of Kalimos", "Baku the Mooneater", "Cat Trick",
				"Dirty Rat", "Druid of the Scythe", "Curse of Rafaam", "Ethereal Peddler", "Hidden Cache", "Arcanosaur",
				"Drakkari Trickster", "Spirit of the Bat", "Meat Wagon", "Prince Valanar", "Kalimos, Primal Lord",
				"Open the Waygate", "Awaken the Makers", "Direhorn Hatchling", "Tinkertown Technician",
				"Bellringer Sentry", "Spreading Plague", "Fatespinner", "Healing Wave", "Corpse Raiser",
				"Twilight Geomancer", "Spectral Pillager", "Prince Taldaram", "Witchwood Grizzly",
				"Twilight Darkmender", "Carnivorous Cube", "Krul the Unshackled", "Venomstrike Trap",
				"Wandering Monster", "Cathedral Gargoyle", "Blood Knight", "Hex Lord Malacrass", "Bolvar, Fireblood",
				"Lesser Sapphire Spellstone", "Spirit of the Raptor", "Crystalline Oracle", "Harrison Jones",
				"Professor Putricide", "Shifting Shade", "Mana Bind", "Varian Wrynn", "Ratcatcher", "Thunderhead",
				"Renounce Darkness", "Sudden Betrayal", "Gurubashi Hypemon", "Sacred Trial", "Blood of The Ancient One",
				"Seadevil Stinger", "Prince Keleseth", "Big Bad Voodoo", "Armored Warhorse", "Darnassus Aspirant",
				"Shadowcaster", "Undercity Huckster", "Void Terror", "Nightmare Amalgam", "Nefarian",
				"Reckless Experimenter", "Cheat Death", "Lynessa Sunsorrow", "Spellbender", "Lifedrinker", "Rat Trap",
				"Illuminator", "Toki, Time-Tinker", "Mark of the Loa", "Blackhowl Gunspire", "Spectral Cutlass",
				"Spellshifter", "Coghammer", "Effigy", "Masked Contender", "A New Challenger...", "Recombobulator",
				"Akali, the Rhino", "Fight Promoter", "Blast Wave", "Dart Trap", "Chittering Tunneler",
				"Grand Archivist", "Griftah" };
...
assertTrue(!CardBuilder.getCard("jan'alai, the dragonhawk").getTags().contains(TagBuilder.getTags().get("STAT_MODIFY")));//[x]battlecry: if your hero power dealt 8 damage this game, summon ragnaros the firelord.@ ({0} left!)@ (ready!) - beast
...
//assertTrue("RESTORE_HEALTH");
		...
		assertTrue(CardBuilder.getCard("zandalari templar").getTags().contains(TagBuilder.getTags().get("STAT_MODIFY")));//[x]battlecry: if you've restored 10 health this game, gain +4/+4 and taunt.@ ({0} left!)@ (ready!) -
...
assertTrue(!CardBuilder.getCard("enhance-o mechano").getTags().contains(TagBuilder.getTags().get("STAT_MODIFY")));//battlecry: give your other minions windfury, taunt, or divine shield (at random). - mechanical
...
assertTrue(!CardBuilder.getCard("shirvallah, the tiger").getTags().contains(TagBuilder.getTags().get("STAT_MODIFY")));//[x]divine shield, rush, lifesteal costs (1) less for each mana you've spent on spells. - beast
...
assertTrue(!CardBuilder.getCard("curious glimmerroot").getTags().contains(TagBuilder.getTags().get("STAT_MODIFY")));//[x]battlecry: look at 3 cards. guess which one started in your opponent's deck to get a copy of it. -
		...
		assertTrue(CardBuilder.getCard("lady in white").getTags().contains(TagBuilder.getTags().get("STAT_MODIFY")));//[x]battlecry: cast 'inner fire' �on every minion in your deck� (set attack equal to health). -
//CHEAP SPELLS
...
assertTrue(!CardBuilder.getCard("spirit of the frog").getTags().contains(TagBuilder.getTags().get("STAT_MODIFY")));//[x]stealth for 1 turn. whenever you cast a spell, draw a spell from your deck that costs (1) more. -
		assertTrue(CardBuilder.getCard("cult sorcerer").getTags().contains(TagBuilder.getTags().get("STAT_MODIFY")));//[x]spell damage +1 after you cast a spell, give your c'thun +1/+1 (wherever it is). -
// CHEAP? SPELLS
...
assertTrue(!CardBuilder.getCard("mecha'thun").getTags().contains(TagBuilder.getTags().get("DESTROY")));//[x]deathrattle: if you have no cards in your deck, hand, and battlefield, destroy the enemy hero. - mechanical
		...
		assertTrue(CardBuilder.getCard("doomcaller").getTags().contains(TagBuilder.getTags().get("STAT_MODIFY")));//battlecry: give your c'thun +2/+2 (wherever it is). if it's dead, shuffle it into your deck. -
...
assertTrue(!CardBuilder.getCard("spirit of the dead").getTags().contains(TagBuilder.getTags().get("STAT_MODIFY")));//[x]stealth for 1 turn. after a friendly minion dies, shuffle a 1-cost copy of it into your deck. -
...
//assertTrue(!CardBuilder.getCard("bomb squad").getTags().contains(TagBuilder.getTags().get("DAMAGE_YOU???")));//[x]battlecry: deal 5 damage to an enemy minion. deathrattle: deal 5 damage to your hero. -		
...
assertTrue(!CardBuilder.getCard("bomb squad").getTags().contains(TagBuilder.getTags().get("STAT_MODIFY")));//[x]battlecry: deal 5 damage to an enemy minion. deathrattle: deal 5 damage to your hero. -
...
assertTrue(!CardBuilder.getCard("the darkness").getTags().contains(TagBuilder.getTags().get("STAT_MODIFY")));//[x]starts dormant. battlecry: shuffle 3 candles into the enemy deck. when drawn, this awakens. -
...
assertTrue(!CardBuilder.getCard("jade idol").getTags().contains(TagBuilder.getTags().get("STAT_MODIFY")));//choose one - summon a{1} {0} jade golem; or shuffle 3 copies of this card into your deck. -
...
assertTrue(!CardBuilder.getCard("leyline manipulator").getTags().contains(TagBuilder.getTags().get("STAT_MODIFY")));//battlecry: if you're holding any cards that didn't start in your deck, reduce their cost by (2). - elemental
...
assertTrue(!CardBuilder.getCard("blade of c'thun").getTags().contains(TagBuilder.getTags().get("STAT_MODIFY")));
...
assertTrue(!CardBuilder.getCard("genn greymane").getTags().contains(TagBuilder.getTags().get("STAT_MODIFY")));//[x]start of game: if your deck has only even- cost cards, your starting hero power costs (1). -
		...
		assertTrue(CardBuilder.getCard("seaforium bomber").getTags().contains(TagBuilder.getTags().get("STAT_MODIFY")));//[x]battlecry: shuffle a bomb into your opponent's deck. when drawn, it explodes for 5 damage. -
		...
		assertTrue(CardBuilder.getCard("da undatakah").getTags().contains(TagBuilder.getTags().get("STAT_MODIFY")));//[x]battlecry: gain the deathrattle effects of 3 friendly minions that died this game. -
		...
		assertTrue(CardBuilder.getCard("frost lich jaina").getTags().contains(TagBuilder.getTags().get("STAT_MODIFY")));//[x]battlecry: summon a 3/6 water elemental. your elementals have lifesteal this game. -
		...
		assertTrue(CardBuilder.getCard("explosive runes").getTags().contains(TagBuilder.getTags().get("STAT_MODIFY")));//secret: after your opponent plays a minion, deal 6 damage to it and any excess to their hero. -
		...
		assertTrue(CardBuilder.getCard("spirit of the tiger").getTags().contains(TagBuilder.getTags().get("STAT_MODIFY")));//[x]stealth for 1 turn. after you cast a spell, summon a tiger with stats equal to its cost. -
		...
		assertTrue(CardBuilder.getCard("lord godfrey").getTags().contains(TagBuilder.getTags().get("STAT_MODIFY")));//[x]battlecry: deal 2 damage to all other minions. if any die, repeat this battlecry. -
		...
		assertTrue(CardBuilder.getCard("inkmaster solia").getTags().contains(TagBuilder.getTags().get("STAT_MODIFY")));//[x]battlecry: if your deck has no duplicates, the next spell you cast this turn costs (0). -
		...
		assertTrue(CardBuilder.getCard("tuskarr jouster").getTags().contains(TagBuilder.getTags().get("STAT_MODIFY")));//battlecry: reveal a minion in each deck. if yours costs more, restore 7 health to your hero. -
		...
		assertTrue(CardBuilder.getCard("stone sentinel").getTags().contains(TagBuilder.getTags().get("STAT_MODIFY")));//battlecry: if you played an elemental last turn, summon two 2/3 elementals with taunt. - elemental
		...
		assertTrue(CardBuilder.getCard("marin the fox").getTags().contains(TagBuilder.getTags().get("STAT_MODIFY")));//battlecry: summon a 0/8 treasure chest for your opponent. (break it for awesome loot!) -
		...
		assertTrue(CardBuilder.getCard("twin emperor vek'lor").getTags().contains(TagBuilder.getTags().get("STAT_MODIFY")));//[x]taunt battlecry: if your c'thun has at least 10 attack, summon another emperor. -
		...
		assertTrue(CardBuilder.getCard("omega mind").getTags().contains(TagBuilder.getTags().get("STAT_MODIFY")));//[x]battlecry: if you have 10 mana crystals, your spells have lifesteal this turn. -
		...
		assertTrue(CardBuilder.getCard("spirit of the shark").getTags().contains(TagBuilder.getTags().get("STAT_MODIFY")));//[x]stealth for 1 turn. your minions' battlecries ��and combos trigger twice.� -
		...
		assertTrue(CardBuilder.getCard("iron juggernaut").getTags().contains(TagBuilder.getTags().get("STAT_MODIFY")));//battlecry: shuffle a mine into your opponent's deck. when drawn, it explodes for 10 damage. - mechanical
		...
		assertTrue(CardBuilder.getCard("finja, the flying star").getTags().contains(TagBuilder.getTags().get("STAT_MODIFY")));//[x]stealth whenever this attacks and kills a minion, summon 2 �murlocs from your deck. - murloc
		...
		assertTrue(CardBuilder.getCard("ancient shade").getTags().contains(TagBuilder.getTags().get("STAT_MODIFY")));//battlecry: shuffle an 'ancient curse' into your deck that deals 7 damage to you when drawn. -
		...
		assertTrue(CardBuilder.getCard("spirit of the rhino").getTags().contains(TagBuilder.getTags().get("STAT_MODIFY")));//stealth for 1 turn. your rush minions are immune the turn they're summoned. -
		...
		assertTrue(CardBuilder.getCard("geosculptor yip").getTags().contains(TagBuilder.getTags().get("STAT_MODIFY")));//at the end of your turn, summon a random minion with�cost�equal�to�your armor (up to 10). -
		...
		assertTrue(CardBuilder.getCard("chillmaw").getTags().contains(TagBuilder.getTags().get("STAT_MODIFY")));//[x]taunt deathrattle: if you're holding a dragon, deal 3 damage to all minions. - dragon
		...
		assertTrue(CardBuilder.getCard("crazed worshipper").getTags().contains(TagBuilder.getTags().get("STAT_MODIFY")));//taunt. whenever this minion takes damage, give your c'thun +1/+1 (wherever it is). -
		...
		assertTrue(CardBuilder.getCard("glitter moth").getTags().contains(TagBuilder.getTags().get("STAT_MODIFY")));//battlecry: if your deck has only odd-cost cards, double the health of your other minions. - beast
		...
		assertTrue(CardBuilder.getCard("rattling rascal").getTags().contains(TagBuilder.getTags().get("STAT_MODIFY")));//[x]battlecry: summon a 5/5 skeleton. deathrattle: summon one for your opponent. -
		...
		assertTrue(CardBuilder.getCard("tinkmaster overspark").getTags().contains(TagBuilder.getTags().get("STAT_MODIFY")));//[x]battlecry: transform another random minion into a 5/5 devilsaur or a 1/1 squirrel. -
		...
		assertTrue(CardBuilder.getCard("dragoncaller alanna").getTags().contains(TagBuilder.getTags().get("STAT_MODIFY")));//battlecry: summon a 5/5 dragon for each spell you cast this game that costs (5) or more. -
		...
		assertTrue(CardBuilder.getCard("cenarius").getTags().contains(TagBuilder.getTags().get("STAT_MODIFY")));//choose one - give your other minions +2/+2; or summon two 2/2 treants with taunt. -
		...
		assertTrue(CardBuilder.getCard("druid of the saber").getTags().contains(TagBuilder.getTags().get("STAT_MODIFY")));//[x]choose one - transform into a 2/1 with charge; or a 3/2 with stealth. -
		...
		assertTrue(CardBuilder.getCard("blood imp").getTags().contains(TagBuilder.getTags().get("STAT_MODIFY")));//[x] stealth. at the end of your turn, give another random friendly minion +1 health. - demon
		...
		assertTrue(CardBuilder.getCard("splintergraft").getTags().contains(TagBuilder.getTags().get("STAT_MODIFY")));//[x]battlecry: choose a friendly minion. add a 10/10 copy to your hand that costs (10). -
		...
		assertTrue(CardBuilder.getCard("freezing trap").getTags().contains(TagBuilder.getTags().get("STAT_MODIFY")));//secret: when an enemy minion attacks, return it to its owner's hand. it costs (2) more. -
		...
		assertTrue(CardBuilder.getCard("raza the chained").getTags().contains(TagBuilder.getTags().get("STAT_MODIFY")));//[x] battlecry: if your deck has no duplicates, your hero power costs (1) this game. -
		...
		assertTrue(CardBuilder.getCard("mind control tech").getTags().contains(TagBuilder.getTags().get("STAT_MODIFY")));//[x]battlecry: if your opponent has 4 or more minions, take control of one at random. -
		...
		assertTrue(CardBuilder.getCard("lesser amethyst spellstone").getTags().contains(TagBuilder.getTags().get("STAT_MODIFY")));//lifesteal. deal 3 damage to a minion. (take damage from your cards to upgrade.) -
		...
		assertTrue(CardBuilder.getCard("shellshifter").getTags().contains(TagBuilder.getTags().get("STAT_MODIFY")));//[x]choose one - transform into a 5/3 with stealth; or a 3/5 with taunt. -
		...
		assertTrue(CardBuilder.getCard("prince liam").getTags().contains(TagBuilder.getTags().get("STAT_MODIFY")));//[x]battlecry: transform all 1-cost cards in your deck �into legendary minions. -
		...
		assertTrue(CardBuilder.getCard("servant of yogg-saron").getTags().contains(TagBuilder.getTags().get("STAT_MODIFY")));//battlecry: cast a random spell that costs (5) or less (targets chosen randomly). -
		...
		assertTrue(CardBuilder.getCard("mimiron's head").getTags().contains(TagBuilder.getTags().get("STAT_MODIFY")));//at the start of your turn, if you have at least 3 mechs, destroy them all and form v-07-tr-0n. - mechanical
		...
		assertTrue(CardBuilder.getCard("druid of the claw").getTags().contains(TagBuilder.getTags().get("STAT_MODIFY")));//[x]choose one - transform into a 4/4 with charge; or a 4/6 with taunt. -
		...
		assertTrue(CardBuilder.getCard("black cat").getTags().contains(TagBuilder.getTags().get("STAT_MODIFY")));//spell damage +1 battlecry: if your deck has only odd-cost cards, draw a card. - beast
		...
		assertTrue(CardBuilder.getCard("king togwaggle").getTags().contains(TagBuilder.getTags().get("STAT_MODIFY")));//[x]battlecry: swap decks with your opponent. give them a ransom spell to swap back. -
		...
		assertTrue(CardBuilder.getCard("fireworks tech").getTags().contains(TagBuilder.getTags().get("STAT_MODIFY")));//[x]battlecry: give a friendly mech +1/+1. if it has deathrattle, trigger it. -
		...
		assertTrue(CardBuilder.getCard("high priestess jeklik").getTags().contains(TagBuilder.getTags().get("STAT_MODIFY")));//[x]taunt, lifesteal when you discard this, add 2 copies of it to your hand. -
		...
		assertTrue(CardBuilder.getCard("the voraxx").getTags().contains(TagBuilder.getTags().get("STAT_MODIFY")));//[x]after you cast a spell on this minion, summon a 1/1 plant and cast another copy on it. -
		...
		assertTrue(CardBuilder.getCard("belligerent gnome").getTags().contains(TagBuilder.getTags().get("STAT_MODIFY")));//[x]taunt battlecry: if your opponent has 2 or more minions, gain +1 attack. -
		...
		assertTrue(CardBuilder.getCard("book wyrm").getTags().contains(TagBuilder.getTags().get("STAT_MODIFY")));//battlecry: if you're holding a dragon, destroy an enemy minion with 3 or less attack. - dragon
		...
		assertTrue(CardBuilder.getCard("malfurion the pestilent").getTags().contains(TagBuilder.getTags().get("STAT_MODIFY")));//[x]choose one - summon 2 poisonous spiders; or 2 scarabs with taunt. -
		...
		assertTrue(CardBuilder.getCard("ozruk").getTags().contains(TagBuilder.getTags().get("STAT_MODIFY")));//[x]taunt battlecry: gain +5 health for each elemental you played last turn. - elemental
		...
		assertTrue(CardBuilder.getCard("star aligner").getTags().contains(TagBuilder.getTags().get("STAT_MODIFY")));//[x]battlecry: if you control 3 minions with 7 health, deal 7 damage to all enemies. -
		...
		assertTrue(CardBuilder.getCard("fal'dorei strider").getTags().contains(TagBuilder.getTags().get("STAT_MODIFY")));//[x]battlecry: shuffle 3 ambushes into your deck. when drawn, summon a 4/4 spider. -
		...
		assertTrue(CardBuilder.getCard("druid of the swarm").getTags().contains(TagBuilder.getTags().get("STAT_MODIFY")));//choose one - transform into a 1/2 with poisonous; or a 1/5 with taunt. -
		...
		assertTrue(CardBuilder.getCard("c'thun's chosen").getTags().contains(TagBuilder.getTags().get("STAT_MODIFY")));//[x]divine shield battlecry: give your c'thun +2/+2 (wherever it is). -
		...
		assertTrue(CardBuilder.getCard("cat trick").getTags().contains(TagBuilder.getTags().get("STAT_MODIFY")));//secret: after your opponent casts a spell, summon a 4/2 panther with stealth. -
		...
		assertTrue(CardBuilder.getCard("druid of the scythe").getTags().contains(TagBuilder.getTags().get("STAT_MODIFY")));//[x]choose one - transform into a 4/2 with rush; or a 2/4 with taunt. -
		...
		assertTrue(CardBuilder.getCard("curse of rafaam").getTags().contains(TagBuilder.getTags().get("STAT_MODIFY")));//give your opponent a 'cursed!' card. while they hold it, they take 2 damage on their turn. -
		...
		assertTrue(CardBuilder.getCard("ethereal peddler").getTags().contains(TagBuilder.getTags().get("STAT_MODIFY")));//battlecry: if you're holding any cards from another class, reduce their cost by (2). -
		...
		assertTrue(CardBuilder.getCard("hidden cache").getTags().contains(TagBuilder.getTags().get("STAT_MODIFY")));//secret: after your opponent plays a minion, give a random minion in your hand +2/+2. -
		...
		assertTrue(CardBuilder.getCard("arcanosaur").getTags().contains(TagBuilder.getTags().get("STAT_MODIFY")));//battlecry: if you played an�elemental last turn, deal�3�damage�to�all other minions. - elemental
		...
		assertTrue(CardBuilder.getCard("drakkari trickster").getTags().contains(TagBuilder.getTags().get("STAT_MODIFY")));//[x]battlecry: give each player a copy of a random card from their opponent's deck. -
		...
		assertTrue(CardBuilder.getCard("spirit of the bat").getTags().contains(TagBuilder.getTags().get("STAT_MODIFY")));//stealth for 1 turn. after a friendly minion dies, give a minion in your hand +1/+1. -
		...
		assertTrue(CardBuilder.getCard("prince valanar").getTags().contains(TagBuilder.getTags().get("STAT_MODIFY")));//battlecry: if your deck has no 4-cost cards, gain lifesteal and taunt. -
		...
		assertTrue(CardBuilder.getCard("open the waygate").getTags().contains(TagBuilder.getTags().get("STAT_MODIFY")));//[x]quest: cast 6 spells that didn't start in your deck. reward: time warp. -
		...
		assertTrue(CardBuilder.getCard("awaken the makers").getTags().contains(TagBuilder.getTags().get("STAT_MODIFY")));//quest: summon 7 deathrattle minions. reward: amara, warden of hope. -
		...
		assertTrue(CardBuilder.getCard("direhorn hatchling").getTags().contains(TagBuilder.getTags().get("STAT_MODIFY")));//taunt deathrattle: shuffle a 6/9 direhorn with taunt into your deck. - beast
		...
		assertTrue(CardBuilder.getCard("tinkertown technician").getTags().contains(TagBuilder.getTags().get("STAT_MODIFY")));//battlecry: if you have a mech, gain +1/+1 and add a spare part to your hand. -
		...
		assertTrue(CardBuilder.getCard("spreading plague").getTags().contains(TagBuilder.getTags().get("STAT_MODIFY")));//summon a 1/5 scarab with taunt. if your�opponent has more minions, cast this again. -
		...
		assertTrue(CardBuilder.getCard("fatespinner").getTags().contains(TagBuilder.getTags().get("STAT_MODIFY")));//choose a deathrattle (secretly) - deal 3 damage to all minions; or give them +2/+2. -
		...
		assertTrue(CardBuilder.getCard("healing wave").getTags().contains(TagBuilder.getTags().get("STAT_MODIFY")));//restore 7 health. reveal a minion in each deck. if yours costs more, restore 14 instead. -
		...
		assertTrue(CardBuilder.getCard("corpse raiser").getTags().contains(TagBuilder.getTags().get("STAT_MODIFY")));//[x]battlecry: give a friendly minion "deathrattle: resummon this minion." -
		...
		assertTrue(CardBuilder.getCard("twilight geomancer").getTags().contains(TagBuilder.getTags().get("STAT_MODIFY")));//[x]taunt battlecry: give your c'thun taunt (wherever it is). -
		...
		assertTrue(CardBuilder.getCard("prince taldaram").getTags().contains(TagBuilder.getTags().get("STAT_MODIFY")));//battlecry: if your deck has�no 3-cost cards, transform into a 3/3 copy of a minion. -
		...
		assertTrue(CardBuilder.getCard("witchwood grizzly").getTags().contains(TagBuilder.getTags().get("STAT_MODIFY")));//[x]taunt battlecry: lose 1 health for each card in your opponent's hand. - beast
		...
		assertTrue(CardBuilder.getCard("twilight darkmender").getTags().contains(TagBuilder.getTags().get("STAT_MODIFY")));//battlecry: if your c'thun has at least 10 attack, restore 10 health to your hero. -
		...
		assertTrue(CardBuilder.getCard("carnivorous cube").getTags().contains(TagBuilder.getTags().get("STAT_MODIFY")));//battlecry: destroy a friendly minion. deathrattle: summon 2 copies of it. -
		...
		assertTrue(CardBuilder.getCard("venomstrike trap").getTags().contains(TagBuilder.getTags().get("STAT_MODIFY")));//secret: when one of your minions is attacked, summon a 2/3�poisonous cobra. -
		...
		assertTrue(CardBuilder.getCard("wandering monster").getTags().contains(TagBuilder.getTags().get("STAT_MODIFY")));//secret: when an enemy attacks your hero, summon a 3-cost minion as the new target. -
		...
		assertTrue(CardBuilder.getCard("blood knight").getTags().contains(TagBuilder.getTags().get("STAT_MODIFY")));//battlecry: all minions lose divine shield. gain +3/+3 for each shield lost. -
		...
		assertTrue(CardBuilder.getCard("bolvar, fireblood").getTags().contains(TagBuilder.getTags().get("STAT_MODIFY")));//divine shield after a friendly minion loses�divine shield, gain +2 attack. -
		...
		assertTrue(CardBuilder.getCard("lesser sapphire spellstone").getTags().contains(TagBuilder.getTags().get("STAT_MODIFY")));//summon 1 copy of a friendly minion. @(overload 3 mana crystals to upgrade.) -
		...
		assertTrue(CardBuilder.getCard("spirit of the raptor").getTags().contains(TagBuilder.getTags().get("STAT_MODIFY")));//[x]stealth for 1 turn. after your hero attacks and ��kills a minion, draw a card.�� -
		...
		assertTrue(CardBuilder.getCard("mana bind").getTags().contains(TagBuilder.getTags().get("STAT_MODIFY")));//secret: when your opponent casts a spell, add a copy to your hand that costs (0). -
		...
		assertTrue(CardBuilder.getCard("varian wrynn").getTags().contains(TagBuilder.getTags().get("STAT_MODIFY")));//battlecry: draw 3 cards. put any minions you drew directly into the battlefield. -
		...
		assertTrue(CardBuilder.getCard("thunderhead").getTags().contains(TagBuilder.getTags().get("STAT_MODIFY")));//[x]after you play a card with overload, summon two 1/1 sparks with rush. - elemental
		...
		assertTrue(CardBuilder.getCard("renounce darkness").getTags().contains(TagBuilder.getTags().get("STAT_MODIFY")));//replace your hero power and warlock cards with another class's. the cards cost (1) less. -
		...
		assertTrue(CardBuilder.getCard("gurubashi hypemon").getTags().contains(TagBuilder.getTags().get("STAT_MODIFY")));//battlecry: discover a 1/1 copy of a battlecry minion. it costs (1). -
		...
		assertTrue(CardBuilder.getCard("sacred trial").getTags().contains(TagBuilder.getTags().get("STAT_MODIFY")));//secret: after your opponent has at least 3 minions and plays another, destroy it. -
		...
		assertTrue(CardBuilder.getCard("prince keleseth").getTags().contains(TagBuilder.getTags().get("STAT_MODIFY")));//battlecry: if your deck has�no 2-cost cards, give�all minions in your deck +1/+1. -
		...
		assertTrue(CardBuilder.getCard("big bad voodoo").getTags().contains(TagBuilder.getTags().get("STAT_MODIFY")));//give a friendly minion "deathrattle: summon a random minion that costs (1) more." -
		...
		assertTrue(CardBuilder.getCard("shadowcaster").getTags().contains(TagBuilder.getTags().get("STAT_MODIFY")));//battlecry: choose a friendly minion. add a 1/1 copy to�your hand that costs�(1). -
		...
		assertTrue(CardBuilder.getCard("nefarian").getTags().contains(TagBuilder.getTags().get("STAT_MODIFY")));//battlecry: add 2 random spells to your hand (from your opponent's class). - dragon
		...
		assertTrue(CardBuilder.getCard("reckless experimenter").getTags().contains(TagBuilder.getTags().get("STAT_MODIFY")));//[x]deathrattle minions you play cost (3) less, but die at the end of the turn. -
		...
		assertTrue(CardBuilder.getCard("cheat death").getTags().contains(TagBuilder.getTags().get("STAT_MODIFY")));//secret: when a friendly minion dies, return it to your hand. it costs (2) less. -
		...
		assertTrue(CardBuilder.getCard("spellbender").getTags().contains(TagBuilder.getTags().get("STAT_MODIFY")));//secret: when an enemy casts a spell on a minion, summon a 1/3 as the new target. -
		...
		assertTrue(CardBuilder.getCard("lifedrinker").getTags().contains(TagBuilder.getTags().get("STAT_MODIFY")));//[x]battlecry: deal 3 damage to the enemy hero. restore 3 health to your hero. - beast
		...
		assertTrue(CardBuilder.getCard("rat trap").getTags().contains(TagBuilder.getTags().get("STAT_MODIFY")));//[x]secret: after your opponent plays three cards in a turn, summon a 6/6 rat. -
		...
		assertTrue(CardBuilder.getCard("illuminator").getTags().contains(TagBuilder.getTags().get("STAT_MODIFY")));//if you control a secret at the end of your turn, restore 4 health to your hero. -
		...
		assertTrue(CardBuilder.getCard("mark of the loa").getTags().contains(TagBuilder.getTags().get("STAT_MODIFY")));//choose one - give a minion +2/+4 and taunt; or summon two 3/2 raptors. -
		...
		assertTrue(CardBuilder.getCard("blackhowl gunspire").getTags().contains(TagBuilder.getTags().get("STAT_MODIFY")));//[x]can't attack. whenever this minion takes damage, deal 3 damage to a random enemy. -
		...
		assertTrue(CardBuilder.getCard("spectral cutlass").getTags().contains(TagBuilder.getTags().get("STAT_MODIFY")));//[x]lifesteal whenever you play a card from another class, gain +1 durability. - - weapon
		...
		assertTrue(CardBuilder.getCard("spellshifter").getTags().contains(TagBuilder.getTags().get("STAT_MODIFY")));//[x]spell damage +1 each turn this is in your hand, swap its attack and health. -
		...
		assertTrue(CardBuilder.getCard("coghammer").getTags().contains(TagBuilder.getTags().get("STAT_MODIFY")));//battlecry: give a random friendly minion divine shield and taunt. - - weapon
		...
		assertTrue(CardBuilder.getCard("a new challenger...").getTags().contains(TagBuilder.getTags().get("STAT_MODIFY")));//discover a 6-cost minion. summon it with taunt and divine shield. -
		...
		assertTrue(CardBuilder.getCard("akali, the rhino").getTags().contains(TagBuilder.getTags().get("STAT_MODIFY")));//rush overkill: draw a rush minion from your deck. give it +5/+5. - beast
		...
		assertTrue(CardBuilder.getCard("fight promoter").getTags().contains(TagBuilder.getTags().get("STAT_MODIFY")));//[x]battlecry: if you control a minion with 6 or more �health, draw two cards. -
		...
		assertTrue(CardBuilder.getCard("blast wave").getTags().contains(TagBuilder.getTags().get("STAT_MODIFY")));//deal 2 damage to�all minions. overkill: add a random mage spell to your hand. -
		...
		assertTrue(CardBuilder.getCard("dart trap").getTags().contains(TagBuilder.getTags().get("STAT_MODIFY")));//secret: after an opposing hero power is used, deal 5 damage to a random enemy. -
		...
		assertTrue(CardBuilder.getCard("griftah").getTags().contains(TagBuilder.getTags().get("STAT_MODIFY")));//[x]battlecry: discover two cards. give one to your opponent at random. -
